...
Calendar strategic, bazat pe date reale de engagement (Buffer 9.6M posts,
Hootsuite, Sprout Social, 2026):
- Carusel: Miercuri, 09:00 (ora Romaniei) - fereastra de dimineata/"invatare",
  cel mai bun swipe-through pentru continut care cere atentie activa.
- Postare simpla: Marti si Joi, 12:00 (ora Romaniei) - fereastra de pranz,
  cel mai consecvent semnal de engagement din toate sursele.
...
3. Pentru intrarile "ready" programate azi -> le publica, filtrate dupa
   tipul de postare potrivit ferestrei orare curente (POST_SLOT).
"""
import itertools
import os
import re
...
SINGLE_POST_WEEKDAYS = {1, 3}   # Marti, Joi (ora 12:00)
CAROUSEL_POST_WEEKDAYS = {2}    # Miercuri (ora 09:00)

# Fereastra oranda curenta, seteata de scheduler.yml prin variabila POST_SLOT,
# in functie de care cron a declansat rularea (github.event.schedule).
# "carousel" -> publica doar carusele; "single" -> publica doar postari simple;
# "all" (default, folosit si la rulare manuala) -> publica tot ce e gata azi.
POST_SLOT = os.environ.get("POST_SLOT", "all")
...
def step_1_ingest_new_content():
    ...
    # 1a. Postari simple (imagini din folderul principal) -> Marti/Joi 12:00
    ...
    for img in new_images:
        ...
        print(f"  + [single] {img['name']} -> {slot} 12:00 ({pillar})")

    # 1b. Carusele (subfoldere din "carusele/") -> Miercuri 09:00
    ...
    for folder in new_carousels:
        ...
        print(f"  + [carusel] {folder['name']} ({len(image_ids)} imagini) -> {slot} 09:00 ({pillar})")
...
def step_3_publish_due_posts():
    print(f"-> Verific postarile programate pentru azi (POST_SLOT={POST_SLOT})...")
    due = cal.get_entries_due_today()
    ready_due = [e for e in due if e["status"] == "ready"]

    if POST_SLOT == "carousel":
        ready_due = [e for e in ready_due if e.get("post_type") == "carousel"]
    elif POST_SLOT == "single":
        ready_due = [e for e in ready_due if e.get("post_type", "single") == "single"]
    # "all" (rulare manuala) -> nu filtram, publicam tot ce e gata azi

    if not ready_due:
        print("  Nimic de postat in aceasta fereastra.")
        return
